...
const renderResults = async () => {
  ...
  interactionState.asset?.interactions?.reverse()?.forEach((inter, i) => {
    let isLast = i === 0
    let citations = ''
    inter.citations.forEach((citation) => {
    ...
    const [, requirement, response] = inter.prompt.match(pattern)
    val += `
        <div class="${isLast && 'text-purple'}">
          ...
          ${response}
          <br /> <br /> <br />
          ${marked.parse(retainLineBreaks(inter.response))}
          <br /> <br />
          ${citations}
...
// * Watching for interaction.loading to change doesn't work for some reason, processingPrompt clears on complete so it's equivalent.
watch(
...
// * Fixes edge cases where the processingPrompt is already null.
onMounted(() => {
...
<template>
  <div
    class="smartcheck relative shadow-lg gap-2.5 hideScroll flex overflow-x-hidden flex-col w-full px-4 h-[80vh] md:mb-0 overflow-y-auto bg-white pt-4"
  >
    <p class="text-gray-500 text-s">SmartCheck Evaluation</p>
    ...
    <div class="max-h-[65vh] h-full z-[1]">
      <div
        ...
        v-html="!interactionState.loading ? content : ''"
      />

      <h4
        class="mt-[-75px] w-full flex justify-center items-center py-6"
        :style="{ opacity: interactionState.processingPrompt ? 1 : 0 }"
      >
        Preparing response
        <div class="ml-2 dot" />
        <div class="dot" />
